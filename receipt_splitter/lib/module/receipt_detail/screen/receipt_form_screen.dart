...
class _ReceiptFormScreenState extends State<ReceiptFormScreen>
  ...
  void initState() {
    ...
    if (isNew) {
      ...
      _dateController.text = DateTimeUtil.dayMonthYear(
        receipt.date ?? currentDate,
      );
  ...
  void _addNewItem() async {
    ...
    if (tab == "Participants") {
      participant = await _showEditBottomSheet<Participant>(
        data: null,
        index: 0,
        isParticipant: true,
      );
  ...
  void _updateItem({Participant? participant, MenuItem? item}) {
    if (participant != null) {
      BlocProvider.of<ReceiptFormCubit>(context).updateParticipant(
        participants: receipt.participants,
        participant: participant,
      );
    } else if (item != null) {
      BlocProvider.of<ReceiptFormCubit>(
        context,
      ).updateItem(items: receipt.items, item: item);
    }
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            isNew ? CREATE_RECEIPT : EDIT_RECEIPT,
          ...
          child: BlocConsumer<ReceiptFormCubit, ReceiptFormState>(
            listener: (context, state) {
              if (state is ReceiptFormSaved) {
                context.pushReplacementNamed(
                  ItemsAndPeopleScreen.itemsAndPeople,
                  extra: state.receipt,
                );
              }
            },
            builder: (context, state) {
              if (state is ReceiptFormLoaded) {
                ...
                context.read<ReceiptTypeCubit>().setType(
                  receipt.taxType ?? TaxType.inclusive,
                );
                                ...
                                showDatePicker(
                                  ...
                                  if (value != null) {
                                    ...
                                    _dateController.text =
                                        DateTimeUtil.dayMonthYear(value);
                                  }
                                });
                              },
                              readOnly: true,
                    ...
                    isNew
                        ...
                        : ParticipantsItemWidget(
                          ...
                          onUpdateParticipant: (participant) {
                            _showEditBottomSheet<Participant>(
                              data: participant,
                              index: 0,
                              isParticipant: true,
                            ).then((value) {
                    ...
                    child: InkWell(
                      onTap: () {
                        if (isParticipant) {
                          ...
                          BlocProvider.of<ReceiptFormCubit>(
                            context,
                          ).deleteItem(items: receipt.items, item: item!);
                        }
